...
function dyn_createScreen()
{
	...
	_showProgressBar(true);
	var dialogWidth, editorDiv, designerDiv, designerDivWidth;
	...
	var _dialogOptions = {modal:true, 
			                  ...
			                  buttons :buttonsArr,
			                  resize: function(){
			        			var resize_dialogWidth = $(this).width() - dialogWidth;
			        			var resize_designerDivWidth = designerDivWidth + resize_dialogWidth;
			        			var editorDivWidth = editorDiv.width();
			        			//adjust the size of the designer div, if its greater than the default size
			        			if(resize_designerDivWidth >= designerDivWidth)
			        			{
			        				designerDiv.width(resize_designerDivWidth);
			        			}
			        			/**
			        			 * adjust the size of the editor div
			        			 * if designer div enlarges than the editor div
			        			 */
			        			if(resize_designerDivWidth > editorDivWidth) 
			        			{
			        				editorDiv.width(resize_designerDivWidth);
			        			}
			                  }
		    		         };
	$("#dyn_create_screen_div_"+_pageRef).load(srcURL, curParams, function() {
		_showProgressBar(false);
		dialogWidth = $(this).width();
		editorDiv = $("#editor_div").parent();
		designerDiv = editorDiv.parent();
		designerDivWidth = designerDiv.width();
	});
...
function loadGeneratorData(screenId,createFrom)
{
	var _screenId = (screenId == null || screenId=="")?-1:screenId;
	var params = {"criteria.screenId":_screenId};
	/**
	 * [TP#1047223] -SBI200398 - IMPL; SMBL; Dynamic file - Create From Issue
	 * added createFrom into param
	 */
	if(isNotNullAndNotUndefined(createFrom) && createFrom==true)
	{
		params["dynScreenCreatorCO.createFrom"] = createFrom;
	}
	$.ajax({
		  ...
		  success: function(data){
			  		 // set the dynamic screen width for editor area
			  		 var dynScreenWidth = data.dynScreenWidth;
			  		 var designerDivWidth = $("#editor_div").parent().parent().width();
			  		 var maxWidth = (designerDivWidth > dynScreenWidth)?designerDivWidth:dynScreenWidth;
			  		 $("#editor_div").parent().width(maxWidth);
			  		 //End- set the dynamic screen width for editor area
