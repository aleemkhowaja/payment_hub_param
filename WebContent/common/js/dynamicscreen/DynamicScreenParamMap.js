...
function dynScreenParamMapGrid_mapTypeChanged()
{
	...
	enableDisableSelectionTypeDynScr();
...
function adjustMapValueResultList(mapTypeDesc,mapElementType)
{
	if(mapTypeDesc == '1')
	{
		jQuery.globalEval("options_liveSearch_sysParamElmDynScrnMapDet_MAP_VALUE_dynScreenParamMapGrid_Id_"  + currentPageRef + ".resultList = 'FLD_IDENTIFIER:sysParamElmDynScrnMapDet.MAP_VALUE_lookuptxt,FIELD_KEY_LABEL_CODE:mapValueDesc,FROM_SOURCE:fromSource'");
...
//TP#934567 Button Customization Dynamic Screen Action Assignment - Parameter from Grid on Source Screen
function enableDisableSelectionTypeDynScr()
{
	var $table = $("#dynScreenParamMapGrid_Id_" + currentPageRef);
	var selectedRowId = $table.jqGrid('getGridParam', 'selrow');
	var selectedRowObject = $table.jqGrid('getRowData', selectedRowId);
	var fromSource = selectedRowObject["fromSource"];
	var delimiter = selectedRowObject["sysParamElmDynScrnMapDet.DELIMITER"];
	if("GRID" == fromSource)
	{
		$table.jqGrid('setCellReadOnly', selectedRowId, "sysParamElmDynScrnMapDet.DELIMITER","false");
		$table.jqGrid('setCellRequired', selectedRowId, "sysParamElmDynScrnMapDet.DELIMITER","true");
		$table.jqGrid('setCellReadOnly', selectedRowId, "sysParamElmDynScrnMapDet.SELECTION_TYPE","false");
	}else
	{
		$table.jqGrid('setCellValue', selectedRowId,"sysParamElmDynScrnMapDet.DELIMITER",'');
		$table.jqGrid('setCellValue', selectedRowId,"sysParamElmDynScrnMapDet.SELECTION_TYPE","");
		$table.jqGrid('setCellReadOnly', selectedRowId, "sysParamElmDynScrnMapDet.DELIMITER","true");
		$table.jqGrid('setCellRequired', selectedRowId, "sysParamElmDynScrnMapDet.DELIMITER","false");
		$table.jqGrid('setCellReadOnly', selectedRowId, "sysParamElmDynScrnMapDet.SELECTION_TYPE","true");
	}
}
