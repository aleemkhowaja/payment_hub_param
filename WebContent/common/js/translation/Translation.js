...
function saveTransDet(allData, shown)
{
	...
	var addArgs = {}; 
	var _afterValidMethod= "_afterValidateErrorMsg";
	var langDescTooltipIdentifier =  currGrid + "#langDesc";
	var lblValueTransTooltipIdentifier = currGrid + "#lblValueTrans";
	var grpValueTransTooltipIdentifier = currGrid + "#grpValueTrans";
	...
	for (var j = 0; j < rowsLen; j++)
	{
		...
		if(currGrid == 'labelTransTbl_IdL')
		{
			...
			{
				addArgs["_elementFocusId"] = langDescTooltipIdentifier;
				_showErrorMsg(invalid_lang_key,error_msg_title,null ,null, _afterValidMethod,addArgs);
				_showProgressBar(false);
				return "invalid";
			}
			
			var transValue = myObject["lblValueTrans"];
			if(isEmptyString(transValue))
			{
				addArgs["_elementFocusId"] = lblValueTransTooltipIdentifier;
				_showErrorMsg(please_fill_empty_fields,error_msg_title,null ,null, _afterValidMethod,addArgs);
				_showProgressBar(false);
				return "invalid";
			}
		}
		else
		{
			...
			transValue = myObject["grpValueTrans"];
			if(isEmptyString(transValue))
			{
				addArgs["_elementFocusId"] = grpValueTransTooltipIdentifier;
				_showErrorMsg(please_fill_empty_fields,error_msg_title,null ,null, _afterValidMethod,addArgs);
				_showProgressBar(false);
				return "invalid";
			}
