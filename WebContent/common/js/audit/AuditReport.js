	...
	$(document).ready(
			function() {
        		...
        		if(typeof preCallRes == "undefined" || preCallRes == "undefined" || preCallRes)
        		{
    				...
    				////886121 removed checking on customization to handle all screens
    				if(typeof fromScreen!= "undefined" && fromScreen!=null && fromScreen!="" && fromScreen!="null" )
    				{
    					if( fromScreen =="CUST")
    					{
    						_auditTrxProgRef = "SCRSETTCONF";
    					}
    					else if ( fromScreen =="TRXALERT" )
    					{
        					_auditTrxProgRef = _auditTrxProgRef + "_ALERT";
    					}
    					else
    					{
    						_auditTrxProgRef = fromScreen;
    					}
    				}
...
function showAuditReportDetail(rowindex) {
	...
	var lookupUrl = jQuery.contextPath	+ '/path/audit/audit_loadAuditReportDetailsPage';
	var arr={};
	arr["appName"] = appName;
	arr["progRef"] = progRef;
	arr["trxNbr"] =trxNbr;
	...
	$("#auditReportDtlDiv").load(lookupUrl, arr, function(){});
}
