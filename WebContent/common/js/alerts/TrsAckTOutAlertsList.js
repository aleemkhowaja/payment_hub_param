// TP 1074791 - Related to BTI190040 - Alert popup after snooze - Large list of Alerts Hangs the page - Need Flipping
function trxAlerts_subscribeEvent(pageRef)
{
	var grid = $("#trsAckTOutAlertGrid_Id_"+pageRef);
	grid.subscribe("trsAckTOutAlertGrid_Id_CompleteTopics",function(){
		 alertGrid_Id_CompleteTopics();
	});
	grid.subscribe("trsAckTOutAlertGrid_Id_onPaging",function(obj){
		trsAckTOutAlertGrid_Id_onPaging(obj);
	});
	
	// Manage the changing of grid page manually from input without pressing enter, and add prevalue attribute 
	// in order to reset the value to the old one when selecting rows and didn't confirm proceeding.
	var pgInput = $("td#trsAckTOutAlertGrid_Id_"+alertsPageRef+"_pager_center input.ui-pg-input");
	pgInput.on("focus",function(e){
		pgInput.attr("prevalue",pgInput.val());
	});
	pgInput.on("focusout",function(e){
		pgInput.val(pgInput.attr("prevalue"));
		grid.jqGrid('setGridParam', {page: pgInput.attr("prevalue")});
	});
}

//TP 1074791 - Related to BTI190040 - Alert popup after snooze - Large list of Alerts Hangs the page - Need Flipping
function trsAckTOutAlertGrid_Id_onPaging(obj)
{
	var pgInput = $("td#trsAckTOutAlertGrid_Id_"+alertsPageRef+"_pager_center input.ui-pg-input");
	
	var grid = $("#trsAckTOutAlertGrid_Id_"+alertsPageRef);
	var selectedRows = grid.jqGrid('getGridParam', 'selarrrow');
	var selectedRowsCount = selectedRows.length;
	if(isNotNullAndNotUndefined(selectedRowsCount) && selectedRowsCount > 0)
	{
		// stop page navigation until confirmation
		obj.originalEvent.returnVal = "stop";
		// get next page to proceed with if the user confirm
		var nextPg = grid.jqGrid('getGridParam', 'page');
		
		// get record count for grid to be set to cover case when the user change the number of record to display
		var reccount = grid.jqGrid('getGridParam', 'reccount');
		var rowList = grid.jqGrid('getGridParam', 'rowList');
		// when choose a value to view number of records in the grid more than the total number then the reccount 
		// return the total number of records, so we should choose least number bigger than the total number exist in choices.
		if(rowList.indexOf(reccount) == -1){
			for (var i = 0; i < rowList.length; i++) {
				if(rowList[i] > reccount){
					reccount = rowList[i];
					break;
				}
			}
		}
		_showConfirmMsg(your_selection_will_be_lost_key, warning_msg_title,
				function(confirmChoice, theArgs) {
					if (confirmChoice) {
						// proceed with pagination
						grid.jqGrid('setGridParam', {page: nextPg});
						grid.trigger("reloadGrid");
					}
					else
					{
						pgInput.attr("prevalue",pgInput.val());
						// reselect rows since the selection is reset when we stop the event (obj.originalEvent.returnVal = "stop")
						for (var i = 0; i < selectedRowsCount; i++) {
						    grid.jqGrid('setSelection', selectedRows[i], true);
						}
						// reselect record count for grid when the user didn't confirm
						grid.jqGrid('setGridParam', {rowNum: reccount});
						$('td#trsAckTOutAlertGrid_Id_'+ alertsPageRef +'_pager_center select.ui-pg-selbox option[value=' + reccount + ']').prop('selected', true);
					}
				});
	}
	else
			...
			success: function(data)
					{
						...
						if(isLoginAlertEnabled != undefined && isLoginAlertEnabled == 'true')
						{
							...
							var errorMessage = data["alertsSC"]["errorMessage"];
							if(isNotNullAndNotUndefined(errorMessage))
							{
								_showErrorMsg(errorMessage, info_msg_title);
							}
							else
							{
								...
								if(todoAlert != undefined && (todoAlert == '@LA' || todoAlert == '@LAS'))
								{
...
/**
 * Common Method to be called from DB defined JS to opne the Common Description Popup
 * Based on TP id 1062906
 */	
function trsAckTOutAlertGrid_openCommonAlertDescription(parameters)
{
    if(parameters != undefined && parameters != null)
    {
        trsAckTOutAlertGrid_openTrxDetails(parameters.todoExcepEnglish,parameters.todoCode,parameters.todoLine,'');
    }
}
