...
function ALERTS_OPEN_ITEM_reloadTrsAckTOutGrid(pageRef, gridParam)
{
	...
	if( $table != null && $table != undefined)
	{
		...
		// TP #951791 - ABSAI200031 - Landing page keeps on refreshing
		if(typeof alertsPageRef != "undefined" && alertsPageRef != null)
		{
...
function ALERTS_OPEN_ITEM_approve(confirmResponse)
{
	...
	$.ajax({
		 ...
		 success: function(data){
			...
			// Add checking on statusChanged flag to close the app/rej dialog and refresh alert grid in case of alert status changed
			if(data["_error"] == undefined || data["_error"] == null || data["statusChanged"] == "true")
		 	{	
			...
			{
				var ALERTS_OPEN_ITEM_errorCallBack = data['_errorCallBackFunc'];
				if(ALERTS_OPEN_ITEM_errorCallBack != undefined && ALERTS_OPEN_ITEM_errorCallBack != null && ALERTS_OPEN_ITEM_errorCallBack != '')
				{
					eval(ALERTS_OPEN_ITEM_errorCallBack + '(data)');
				}
			}
		 },
		 beforeSend:function()
