...
function btnCustOperation_onRowSelTopic(rowObj)
{
	...
	if(operationType == '')
	{
		operationType = selectedRowObject["operationDesc"];
	...
	/*TP#983067 Option to load JS Method to be called from js File located on the server*/
	 ...
	 controlScriptUrlCellAccess($table, operationType, serviceType);
	 
	 // TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
	 enableDisableComponentIdColumn($table, operationType, serviceType, selectedRowId);
	 
	var serviceType = selectedRowObject["imImalApiVO.SERVICE_TYPE"];
	if(serviceType == '')
	{
		serviceType = selectedRowObject["serviceTypeDesc"];
...
/*TP#983067 Option to load JS Method to be called from js File located on the server*/
/**
 * change even handler for script_url cells
 * @param
 * @returns
 */
function scriptUrlCellChanged()
{
	var $table = $("#buttonCustomizationActionsGrid_Id_" + currentPageRef);
	var selectedRowId =$table.jqGrid('getGridParam', 'selrow');
	
	var trimmedUri = $table.jqGrid('getRowData', selectedRowId)["sysParamBtnCustActionsVO.SCRIPT_URL"].trim();
	$table.jqGrid('setCellValue',selectedRowId,"sysParamBtnCustActionsVO.SCRIPT_URL",trimmedUri);
	
	var rowObject = $table.jqGrid('getRowData', selectedRowId);
	var scriptUri = rowObject["sysParamBtnCustActionsVO.SCRIPT_URL"];
	var rowNumber = $table.jqGrid()[0].rows[selectedRowId].rowIndex;
	var invalidFileExtension='';
	if(scriptUri.length > 0 && !isJSFile(scriptUri))
	{
		var errMessage='';
		errMessage += scriptUri +"\r\n";
		errMessage += invalid_js_file_key;
		$table.jqGrid('setCellValue',selectedRowId,"sysParamBtnCustActionsVO.SCRIPT_URL","");
		_showErrorMsg(errMessage , warning_msg_title);
	}
	
	function isJSFile(url)
	{
		var jsFileRegPattern = /^(?!.*\.\.\/.*)([a-zA-Z0-9\s_\/\\.\-\(\)]*)([a-zA-Z0-9]\.js)$/i;
		return jsFileRegPattern.test(url);
	}
}

/**
 * shows ,or hides and clears the script_url column according to selected operation and service
 * @param $table
 * @param operationType
 * @param serviceType
 * @param rowId
 * @returns
 * note: if rowId is not passed, it will take the Id of selectedRow
 */
function controlScriptUrlCellAccess($table, operationType,serviceType,rowId)
{
	if(operationType === "A" && serviceType === "C")
	{
		enableScriptUrlCell($table,rowId);
	}
	else
	{
		disableAndClearScriptUrlCell($table,rowId);
	}
	
	function enableScriptUrlCell($table,rowId)
	{
		if(rowId === undefined || rowId === null)
		{
			rowId = $table.jqGrid('getGridParam', 'selrow');
		}
		$table.jqGrid('setCellReadOnly', rowId, "sysParamBtnCustActionsVO.SCRIPT_URL","false");
	}
	function disableAndClearScriptUrlCell($table,rowId)
	{
		if(rowId === undefined || rowId === null)
		{
			rowId = $table.jqGrid('getGridParam', 'selrow');
		}
		$table.jqGrid('setCellReadOnly', rowId, "sysParamBtnCustActionsVO.SCRIPT_URL","true");
		$table.jqGrid('setCellValue', rowId,"sysParamBtnCustActionsVO.SCRIPT_URL",'');
	}
}
...
function btnCustOperation_actionTypeChanged()
{
	...
	// TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
	enableDisableComponentIdColumn($table, "A", serviceType, selectedRowId);
	
	/*TP#983067 Option to load JS Method to be called from js File located on the server*/
	controlScriptUrlCellAccess($table , "A", serviceType);
}

// TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
function enableDisableComponentIdColumn(table, operationType, serviceType, rowId)
{
	// 'I' for report, 'A' for action
	if(serviceType === "I" && operationType === "A")
	{
		table.jqGrid('setCellReadOnly', rowId, "sysParamBtnCustActionsVO.COMPONENT_ID","false");
	}
	else
	{
		table.jqGrid('setCellReadOnly', rowId, "sysParamBtnCustActionsVO.COMPONENT_ID","true");
		table.jqGrid('setCellValue', rowId,"sysParamBtnCustActionsVO.COMPONENT_ID",'');
	}
}
...
function btnCustOperation_BtnFormatter(cellValue, options, rowObject) 
{
	...
	{
		var theDetLabel = details_key;
		if(theDetLabel === "")
		{
			var hiddenDetInp = document.getElementById("details_key_id");
			theDetLabel = (hiddenDetInp? hiddenDetInp.value: "Details")
		}
