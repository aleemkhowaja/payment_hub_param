  function arabicCharOnly(evt)
  {
	...
	// check if element not read Only TP1012172
	 if($theElement.attr("readonly") == "undefined" || !$theElement.attr("readonly"))
	 {
	 ...
	 {
		 //TP 1031212 if input > max length do not proceed on key click
		 if (currentval.length >= maxlength 
				 && !(theKeyCode === 8/*backspace*/) 
				 && !(theKeyCode === 46/*delete*/)
				 && !(theKeyCode === 37/*arrow-left*/)
				 && !(theKeyCode === 38/*arrow-up*/)
				 && !(theKeyCode === 39/*arrow-right*/)
				 && !(theKeyCode === 40/*arrow-down*/)
				 && !(theKeyCode === 9/*tab*/)) 
		 {
			 evt.preventDefault();
	         return;
