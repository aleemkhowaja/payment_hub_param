...
( function($) {
	...
	$.externalFileLoader = {
			...
			filesCache : {},
			/**
			 * $.externalFileLoader.require function is inspired from $.struts2_jquery.require function.
			 * It loads application-external javascript files securely using a server action
			 * and caches them if the load is succeeded
			 * @param {string} fileId -  used for caching
			 * @param {string} actionUrl - server action url to call
			 * @param {array} queryParamsObjArr - array of objects with structure {name:[name] ,value:[value]}
			 * @param {boolean} isAsync - decides whether to make sync or async ajax call
			 * @param {function] callBack - a function to call upon success passing to it 
			 * errResult{hasError:[false/true] , errorMessage:[errorMessageFromServer]} as param
			 */
			require : function(fileId, actionUrl, queryParamsObjArr, isAsync , callBack) {
				var self = this;
				if (!self.filesCache[fileId]) 
				{
					if($.type(isAsync) !== "boolean")
					{
						isAsync = false;
					}
					...
					var params = '';
					var urlWithParams ='';
					if(queryParamsObjArr !== undefined
							&& queryParamsObjArr !== null
							&& typeof queryParamsObjArr === 'object')
					{  
						params = $.param(queryParamsObjArr);
						urlWithParams=actionUrl+'?'+params;
					}
					else
					{
						urlWithParams=actionUrl;
					}
					$.ajax( {
						type :"GET",
						url : urlWithParams,
						success :function(data){
							var errResult
							if(isNotNullAndNotUndefined(data._error))
							{
								errMessage = data._error;
								errResult = {hasError:true , errorMessage:data._error};
							}
							else
							{
								// cache upon success
								self.filesCache[fileId] = true;
								errResult = {hasError:false , errorMessage:''};
							}
							successFunction(errResult);
...
callGridDependency = (function(gridId,dependency, dependencySrc,parameter,rowid,afterDepEvent,currColName, gridCell,triggerSaveRow,aftersavefunc )
{
		...
		//TP#1032791 Retreive previous value
		var preValue ="";
		var currSavedRow = $("#"+gridId)[0].p.savedRow;
		...
		if(typeof currSavedRow != "undefined" && !(currSavedRow.length < 0 && typeof currSavedRow[0].id == "undefined"))
		{
			var tempJsonString = JSON.stringify(currSavedRow);
			if(tempJsonString.length > 2)
			{
				tempJsonString = tempJsonString.replace("[","");
				tempJsonString = tempJsonString.replace(/]$/,""); //replace the last ] of the json string
				var finalJsonObj = eval( '(' + tempJsonString + ')');
				preValue = finalJsonObj[currColName];
			}
		}
				...
				if(j == 0 && depOnSameElemExst && dataValue === "" && currColValue != ""
					&& (data["_disRevertOldVal"] == null || data["_disRevertOldVal"] !== "true"))
					{
						...
						else if(_savedRow[0].id == selRowId )
						{
							...
							if(typeof $elt == "undefined" || typeof $elt.attr("prevValue") == "undefined")
                            {
                             //TP#1032791 set previous value of selected row
								dataValue = preValue ;
                            } 
							else
								dataValue = $elt.attr("prevValue");
 ...
 */ 
 liveElem = (function (value, options,tblId) {
  ...
  var multiResultInput = (typeof colProp["multiResultInput"]== "undefined") ? '' : _autoId+"_"+colProp["multiResultInput"]+(startsWith(_myrowId,'new')? "" : "_"+tblId);
   ...
   */
  // TP #935078 - System is not allowing to edit link document details in Update After Approve Screen
  // Remove recReadOnly since already checked in class(PathGridColumn.java)
  var readOnlyMode = (typeof colProp["readOnlyMode"]== "undefined") ? '' : colProp["readOnlyMode"];
  var readOnlyModeValue= (typeof colProp["readOnlyModeValue"]== "undefined" && readOnlyMode=="true") ? 'true'  : (typeof colProp["readOnlyModeValue"]== "undefined" && readOnlyMode=="false") ? 'false' : colProp["readOnlyModeValue"];
  //[TP#1043972] OnChange Event For Grid Column- Editable Grid Customization Enhancements
  var customBtnDataValue = (typeof colProp["customBtnData"] == "undefined" || isEmptyString(colProp["customBtnData"])) ? '' : colProp["customBtnData"];
  /*
   * 
   */
  if(cssStyle != '')
  ...
  var elemScriptConstruct = " <script type='text/javascript'>\n"+
	...
	" options_liveSearch_"+repColName+".customBtnData = '"+customBtnDataValue+"'; \n" +
   	" </script>";
...
$.fn.processAfterValid= (function(_methodName, args, additionalArgs)
{
	var valRadioNotSel = false;
	// used to check the validation for none of the radio button is selected
	if(additionalArgs != undefined && additionalArgs.validateNotSelectRadios)
		valRadioNotSel = true;
	...
	$(this).validate(
		{
			...
			onkeyup: false,
			onclick: false,		
			validateNotSelRadios: valRadioNotSel,
			showErrors: function(errorMap, errorList) {
				...
				//TP 1048293 check if there is some Javascript calls (JS methods, Reports) running after as result of button Customization response, before execution
				var isJSCallOnBtnCustomizationActive = false;
				if(custmzJSCallsRunning)
				{
					isJSCallOnBtnCustomizationActive = true;
				}
				
		   	if(!isJSCallOnBtnCustomizationActive &&
		   	   ( (isAlertEnabled ==="true" && jQuery.active > (1+jqueryAct)) || (isAlertEnabled !== "true" && jQuery.active > jqueryAct)) )
	    	{
		   		_showProgressBar(false);
...
function openPreviewRepCommon(dataParams,params,autoPrint, callSucc, errorFunc, successArg, errorArg, customReportAction, customReportParams, additionalParams)
{
      ...
      // TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
  	  var componentIdNotExist = isNullOrUndefined(additionalParams) || isNullOrUndefined(additionalParams.componentId) || isEmptyString(additionalParams.componentId, true);
      ...
      else
      {
    	  //TP 317013 if global showPrintDialog checked then need to popup browser print dialog
      	  // Edited - TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement(componentIdNotExist)	
	   	   if(glbl_showPrintPreviewDialog === 'Y' && !openInFrame && componentIdNotExist)
	   	   {
	   		   a_p = 1;
	   	   }
      }
    	...
    	
    {
    	...
    	$.post(url,dataParams,function(HTML){
    		_showProgressBar(false);
    		_resetBtnCustomCallsFlags(additionalParams);
    		if(_jsonReturn)
    		{
    ...
    else
    {
    	...
    	// TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
    	if(!componentIdNotExist)
    	{
    		var result = loadReportInIframeDiv(false, additionalParams.componentId, null);
 ...
 * @param isHtmlReport
 * @param componentID
 * @param HTML should be send if isHtmlReport = true
 * @returns object that contain iFrameID and target only if isHtmlReport is false 
 */
function loadReportInIframeDiv(isHtmlReport, componentID, HTML)
{
	var iFrameID,target = null;
	
	if(isNotNullAndNotUndefined(componentID) && isNotEmptyString(componentID, true))
	{
		// id with pageRef
		var componentId = componentID + "_" + _pageRef;
		var tag = $("#" + componentId);
		if(tag.length == 0)
		{
			// id without pageRef
		...
		if(tag.length > 0)
		{
			...
			var iframeTag = $("iframe#" + componentId + "Iframe");
			// (tagName = "iframe") means that thecomponentId refer to iframe already added by developer,
			// while if (tagName == "div" && iframeTag.length > 0) refer to Iframe created by this function with a previous call
			if(tagName == "iframe" || (tagName == "div" && iframeTag.length > 0) )
			{
				// iframe created by this function with a previous call
				if(tagName == "div")
				{
					componentId = componentId + "Iframe";
	    			tag = iframeTag;
				}
				// iframe created by the developer
				else 
				{
					// set target to iframe name if exist
					if(typeof tag.attr("name") !== "undefined" && tag.attr !== false && tag.attr("name") != "")
					{
						target = tag.attr("name");
					}
					else
					{
						// if name attribute equal to empty or not exist, remove the iframe and create new iframe with same attributes + name attr 
						var parent = tag.parent();
						var attributes = tag.prop("attributes");
						var newiFrame = '<iframe name = "'.concat(componentId + '" ');
						$.each(attributes, function() {
							if(this.name != null && this.name != "name"){
								newiFrame = newiFrame.concat(this.name + '="' + this.value + '" ');
							}
						});
						newiFrame = newiFrame.concat("/>");
						tag.remove();
						$(newiFrame).appendTo(parent);
						tag = $("#" + componentId);
						target = componentId;
					}
				}
				...
				{
					iFrameID = componentId;
				}
			}
			// componentId refer to html div
			else if(tagName == "div")
			{
				// create an iframe tag
				iFrameID = componentId + "Iframe";
				var $iframe = $("<iframe id='"+iFrameID+"' name='"+iFrameID+"' align='middle' width='100%' height='500' >");
    			
				// check if the componentId is for panel has class collapsibleContainer, this case exist when the panel is added from developer side
    			var divPnlContainerId = "div#" + componentId + ".collapsibleContainer";
    			var divPnlContainer = $(divPnlContainerId);
    			
    			// check if the componentId is for panel has a div child with class collapsibleContainer, this case exist when the panel is added in dynamic screen
    			var pnlContainerId = "div#" + componentId + " .collapsibleContainer";
    			// componentId refer to a panel
    			if(divPnlContainer.length > 0 ||  $(pnlContainerId).length > 0)
    			{
    				if(divPnlContainer.length > 0)
    				{
    					pnlContainerId = divPnlContainerId;
    				}
    				
        			var panelcontainerContent;
        			var collapsibleContentSelector = pnlContainerId + " div.collapsibleContainerContent.ui-widget-content";
        			
        			// content div of the panel already exist
        			if($(collapsibleContentSelector).length > 0)
        			{
        				panelcontainerContent = $(collapsibleContentSelector);
        			}
        			else
        			{
        				var collapsiblePnl = $(pnlContainerId);
        				// create the content div of the panel
        				panelcontainerContent = $("<div class = 'collapsibleContainerContent ui-widget-content' />");
        				panelcontainerContent.appendTo(collapsiblePnl);
        			}
        			
        			// add the created iframe tag to the panel container
        			$iframe.appendTo(panelcontainerContent);
        			if(isHtmlReport)
    				{
    			...
    			// componentId refer to a div tag
    			else
        		{
    				// add the created iframe tag to the div tag
    				$iframe.appendTo(tag);
    				if(isHtmlReport)
    				{
    					var iframe = $iframe.get(0);
			...
			// return iFrameID and iframe tag object in case isHtmlReport parameter is false
			if(!isHtmlReport)
			{
				if(null == target)
				{
...
function _resetBtnCustomCallsFlags(theAddParams)
{
	if(theAddParams && theAddParams.fromCustBtnAction)
	{
		custmzJSCallsRunning = false;
	}
}
function manageSuccessJSONReport(args)
{
   ...
   if(returnJsonData!="undefined" && returnJsonData!=null)
   {
	  ...
	  //TP 1016565, filing the arguments passed as empty and available in returned JSON result
	  if(args.successArg != "undefined" && args.successArg != null)
	  {
		  for (var key in args.successArg) 
		  {
			  for (var keyReturned in returnJsonData) 
			  {
				  if(key == keyReturned && (args.successArg[key] == null || args.successArg[key] == "undefined" || args.successArg[key] == ""))
				  {
					  args.successArg[key] = returnJsonData[key];
				  }
			  }
		  } 
	  }
...
function manageSuccessHTMLReport(confirmed,args)
{
	if(confirmed)
	{
		...
		// TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
		if(isNotNullAndNotUndefined(additionalParams) && isNotNullAndNotUndefined(additionalParams.componentId) && isNotEmptyString(additionalParams.componentId, true))
    	{
			loadReportInIframeDiv(true, additionalParams.componentId, HTML);
    	}
		else if(openWindow)
	    {
...
function printSingleAdvice(reportResponse,callBackFunc,callBackArgs,forceCallBack, additionalParams)
{
	if(reportResponse)
	{
		...
		// TP 1062269 Loading Report Assigned by Customization into provided Div or iFrame ID - Customization Enhancement
		if(isNotNullAndNotUndefined(reportResponse) && isNotNullAndNotUndefined(reportResponse.componentId) && isNotEmptyString(reportResponse.componentId, true))
		{
			if(typeof additionalParams === "undefined" || additionalParams == null)
			{
				additionalParams = {};
			}
			additionalParams["componentId"] = reportResponse.componentId;
		}
	...
	else
		{
			$("#"+targetDivId).load(loadDynURL,curParams,function() {
			   ...
			   _divContents.each( function() {
				 ...
				 /**
				  * [TP#1032915] - Dynamic screen additional tab overlapping with system screen button
				  * $(this).hasClass("loading") used to validate the element, if it is loading element then no need to add the height based on it.
				  * compare _currElemPos with _maxElemPos to assign _maxElemPos to the div height 
				  */
				 var _currElemPos = 0;
				 if(_elemTopVal != null && !isNaN(_elemTopVal) && !hasParentPanel && !$(this).hasClass("loading"))
				 {
					 _elemHeightVal = isNaN(_elemHeightVal)?0:_elemHeightVal;
					 _currElemPos = _elemTopVal + _elemHeightVal;
					 if( _currElemPos > _maxElemPos)
					 {
						 _maxElemPos = _currElemPos;
					 }
				 }//End [TP#1032915]
			   });
			   ...
			   {
				   _actualDivHeight = _maxElemPos + 10;
			   }
...
function customBtnActionCall(buttonId,validateForm,proceed,customBtnData)
{
	...
	{	
		/**
		 * [TP#1043972] OnChange Event For Grid Column- Editable Grid Customization Enhancements
		 * get tableId from customBtnData to get selectedRowObject
		 */
		var tableId = customBtnData.gridId;
			    ...
			    if(data["_error"] == null)
		 		{
			    	...
			    	if(clientScriptList != undefined && clientScriptList != null)
			    	{
			    		...
			    		try
			    		{
			    			var paramsMap =data.buttonCustomizationCO.actionsParamMap;//get object containing info about operations
			    			var operationIdsWithScriptList = getOpsWithExtrScrpts(paramsMap);//get operations that have External scripts
			    			if(operationIdsWithScriptList.length > 0)  // there are external scripts to load
			    			{
			    				getExtrScrptFiles(operationIdsWithScriptList);//get scripts.
			    			}
			    			executeFunctions();
			    			openDyScrnAndApplyOutParams();
			    		}
			    		catch(err)
			    		{
			    			_showErrorMsg(err);
	    		    ...
	    		    {
		    			//validate that they are all functions
		    			var undefinedFuncsList = clientScriptList.map( function(funcCall){return (funcCall.indexOf('(')<1)?funcCall:funcCall.substring(0,funcCall.indexOf('('));})
						 .filter(function(func){return !$.isFunction(window[func]);});
		    			
		    			if(undefinedFuncsList.length > 0)//if one at least is not a function don't proceed
		    			{
		    				throw function_not_defined_key+": \r\n"+ undefinedFuncsList.toString().replace(/,/g,"\r\n");
		    			}
	    		    	for(var i=0; i<clientScriptList.length; i++ )
				    	    {
				    	    	if(isNotEmptyString(clientScriptList[i]))
				    	    	{
			    	...
			    	function openDyScrnAndApplyOutParams()
			    	{
				    	...
				    	if(dynScreenId != undefined && dynScreenId != null && dynScreenId >= 0 )
				    	{
							...
							//[TP#1043972] set screen width as set from column custom activity grid
				    		if(customBtnData.screenWidth != null && customBtnData.screenWidth != undefined)
				    		{
				    			screenWidth = customBtnData.screenWidth;
				    		}
				    		var screenHeight = data["buttonCustomizationCO"]["customActionParamCO"]["screenHeight"];
							//[TP#1043972] set screen height as set from column custom activity grid
				    		if(customBtnData.screenHeight != null && customBtnData.screenHeight != undefined)
				    		{
				    			screenHeight = customBtnData.screenHeight;
				    		}
				    		var screenTitle = data["buttonCustomizationCO"]["customActionParamCO"]["screenTitle"];
							//[TP#1043972] set screen title as set from column custom activity grid
				    		if(customBtnData.screenTitle != null && customBtnData.screenTitle != undefined)
				    		{
				    			screenTitle = customBtnData.screenTitle;
				    		}
			    	...
			    	// helping functions
	    			function getOpsWithExtrScrpts(actionsParamMap)
	    			{
	    				function isTrue(text)
	    				{
	    					return text !== undefined && text !== null && text === 'true' ;
	    				}
	    				
	    				var operationIdWithScriptList = [];
	    				// loop through outer map
	    				for( param in actionsParamMap)
	    				{
	    					// loop through inner map
		    				for( inParam in actionsParamMap[param])
		    				{
		    					var action = actionsParamMap[param][inParam];
		    					if(action.apiType === "C" && isTrue(action.scriptUrl))
		    					{
		    						operationIdWithScriptList.push(action.operationId);
		    					}
		    				}
	    				}
	    				return operationIdWithScriptList;
	    			}
	    			
		    		function getExtrScrptFiles(operationIdWithScriptList)
		    		{
		    			var url = jQuery.contextPath + '/path/buttoncustomization/ButtonCustomizationMaint_loadExternalScriptFiles.action';
		    			var numberOfOperations = operationIdWithScriptList.length;
		    			var customizationId = 'dynamicCus_';
		    			for(var i =0 ; i< numberOfOperations ; i++)
		    			{
		    				var buttonId = newCustomBtnData.customBtnId;
		    				var opId = operationIdWithScriptList[i];
		    				var params = [{name:'buttonId',value:buttonId},
		    					{name:'operationId',value:opId},
		    					{name:'_pageRef',value:_pageRef}];
		    				$.externalFileLoader.require(customizationId+buttonId+opId, url, params, false ,function(errResult){
		    					if(errResult.hasError)
		    					{
		    						_showErrorMsg(errResult.errorMessage , warning_msg_title);
		    					}
		    				});
		    			}
		    		}
...
var custmzJSCallsRunning; //TP 1048293 variable to specify if there is any JS,Report call running due to button customization
...
function customBtnActionCall_prepareAdvicesParam(reportResponse)
{
	...
	if(reportResponse != undefined && reportResponse != null)
	{
		...
		paramObj.additionalParams = {fromCustBtnAction:true};
	}
	return paramObj;
}
...
function dynamicScreen_openDynamicScreen(currScreenId,dynamicScreenParamsMapCOList,buttonId,screenWidth,screenHeight,fromPreview,_recReadOnly,screenTitle)
{
	...
	if(isNotNullAndNotUndefined(_pageRef))
	{
		var srcURL = jQuery.contextPath+"/path/dynamicScreen/dynamicScreenMainAction_returnEntityTypeInfos?";
		$.ajax({
			url : srcURL,
			async : false,
			...
			dataType : 'json',
			success : function(data) {
				// passing entityType parameters in request Parameters when opening Dynamic Screen
				$.each(data.screenEntityTypeInfosList, function(index,entity){
					if(entity == "CIFTYPE")
					{
						curParams["cifType"] = returnHtmlEltValue("lookuptxt_cifVO_CIF_TYPE_" + _pageRef);
					}
					else if(entity == "COUNTRYID")
					{
						curParams["countryId"] = returnHtmlEltValue("lookuptxt_cifVO_COUNTRY_" + _pageRef);
					}
				});
			}
		});
	}
	//End - TP#1009625 Dynamic Screen Customized Required
	...
	if(typeof fromPreview!="undefined" && fromPreview!=null && fromPreview)
	{
		...
		//TP#1049922 _pageRef is empty when opening dynamic screen on preview screen
		curParams["_pageRef"] = "";
	}
	...
	//TP#989676 fetch child screen related data and send with request as parameter
	var parentScrIdElem = $("#"+buttonId).parent().parent().find("input[name='scrElemHm.dynamic_screenId']");
	if(parentScrIdElem.length > 0)
	{
		var screenId = parentScrIdElem.attr("value");
		var parentScreenParams = {"screenId":screenId,"buttonId":buttonId};
		curParams["parentScreenParams"] = JSON.stringify(parentScreenParams);
		
		var childScreenData = dynamicScreen_returnChildScreenData(screenId, buttonId);	// fetch child screen data		
		if(childScreenData != null)
		{
			curParams["criteria.sourceData"] = JSON.stringify(childScreenData);
		}
	}
	//End TP#989676
 ...
 * @description return currently opened child screen data
 *  
 * @returns
 */
function dynamicScreen_returnChildScreenData(screenId, buttonId)
{
	var childScreenData = null;
	var childArrEltId = screenId + "_childScreensArray";	//fetch child screens data
	if($("#"+childArrEltId).length > 0)
	{
		var jsonStr = $("#"+childArrEltId).val();
		var screenData = JSON.parse(jsonStr);
		var childScreenKey = screenId + "_" + buttonId;
		//iterate screens data and get current child screen data
		for(var i = 0; i < screenData.length; i++) {
			var obj = screenData[i];
			if(obj[childScreenKey] != null)
			{
				childScreenData = obj[childScreenKey];
				break;
			}
		}
	}
	return childScreenData;
}
