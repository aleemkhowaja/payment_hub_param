					...
					if ( nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
						...
						if(cm[i].editable===true) {
							...
							if(typeof(cm[i].loadOnce)!=="undefined" && cm[i].loadOnce== false  )
								...
								{ cm[i].loadOnce = true; }
							//***************************************************************
							//concatenate gridId at the end of the element id except if element is newly created, liveSearch or Dialog TP 990828
							var opt = $.extend({},{colModel:cm[i]},cm[i].editoptions || {},{id:((startsWith(rowid,'new')||cm[i].colType=="liveSearch"||cm[i].colType=="dialog")?rowid+"_"+nm:rowid+"_"+nm+"_"+$t.p.id),name:nm},{loadOnce:cm[i].loadOnce} ); //adding loadOnce for data retrieval for the columns of type "select"
...
//								}
								 ...
								 * load opts of livesearch and set customBtnData value for livesearch input if exists 
								 */
								if(cm[i].colType == "liveSearch")
								{
		        	                var opts = "options_liveSearch_" + cm[i].name + "_" + $.jgrid.id;
		        	                while(opts.indexOf(".") > -1)
		        	                {
		        	                	opts = opts.replace(".","_");
		        	                }	        	                
	        	                	var liveElem = eval(opts);
	        	                	if(typeof liveElem != "undefined" && liveElem != null && isNotEmptyString(liveElem.customBtnData))
	        	                	{
	        	                		var customBtnDataValue = $.parseJSON(liveElem.customBtnData);
	        	                		$elc.data("customBtnData", customBtnDataValue);	
	        	                	}
								}
							}
		...
		if (editable==="1") {
				...
				if ( nm != 'cb' && nm != 'subgrid' && cm.editable===true && nm != 'rn' && !$(this).hasClass('not-editable-cell')) {
					switch (cm.edittype) {
						case "checkbox":
							 ...
							 */
							if(cm.editoptions && typeof cm.editoptions.value != "undefined") {
					...
					if(cv[0] === false) {
						...
						cv[2] = acquireGridCellTooltipElem($thisElem);
			...
			if (cv[0] === false){
				try {
					...
					/*tp#965931_Focus on Grid Column*/
					$.jgrid.info_dialog($.jgrid.errors.errcap,cv[1],$.jgrid.edit.bClose,infoDlgOpts,cv[2]);
				} catch (e) {
					alert(cv[1]);
				}
				return success;
