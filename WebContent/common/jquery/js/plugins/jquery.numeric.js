            ...
            switch (keyPressedCode)
            {
                ...
                case o.format.decimalChar.charCodeAt(0): // Decimal 
            		...
            		if(keyPressedChar == o.format.decimalChar)//decimal point is being pressed check for number of digits before it to see if it can be allowed
        			{
            			...
            			//1075576 RKFHK200315 - Inv.Deal - Difficulty while entering yield for a deal
            			//check if the value selected is the whole input value to allow decimal pressing
            			if(_value != selectedHtmlText && ((o.maxLenBeforeDec && lenBeforeCurrentPos > o.maxLenBeforeDec) || lenAfterCurrentPos > maxLenAfterDec))
            			{
							event.preventDefault();
	                		event.stopPropagation();
	                		return;
