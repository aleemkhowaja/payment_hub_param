...
( function($) {
	...
	bind : function(el, o) {
		...
		if (el) {
			...
			if(!el) //PathSolutions: in case of submit with progref we are not constructing the input submit itself 
				return;
	...
	anchor : function($elem, o) {
		...
		//incase the element contains a label tag then we add the class to the label in order to exclude coloring it in theme customization.
		if($elem.hasClass("ui-button") && $elem.find("label").length)
		{
			$($elem.find("label")).addClass("path_btn_lbl_theme")
		}
	},
	...
	button : function($elem, o) {
		...
		var isButtonType = ($elem.attr("type") === "button");
		if ((!o.href || o.href === "#" ) && !isButtonType && o.formids !== undefined) {
			self.formsubmit($elem, o, formTopic);
		}
		else {
			if(o.href && o.href !== "#"){
				...
				if (cform !== undefined && !isButtonType) {
					var cf = $(cform);
	...
	tabbedpanel : function($elem, o) {
		...
		para.select = function(event, ui) {
			...
			data.event = event;
			data.ui = ui;
            var _currTabId       = ui.tab.parentNode.id;
            var _currTargetDivId = ui.panel.id; 
            if(_currTabId.indexOf("newTabId_") == 0)
            {
            	tabbedPanel_onSelectDynTab(_currTabId,_currTargetDivId);
            }
            else
            {            	
            	...
            	/**
            	 * [PathSolutions-MarwanMaddah]: added the onSelect Management as topic in the struts2_jquery tabbed panel
            	 */
            	if(o.onselect) {
            		self.publishTopic($elem, o.onselect, data);
            		self.publishTopic($elem, o.onalw, data);
            	}
            }
		...
		/**
		 * [Marwan Maddah]: Additional tabs management
		 */
		if(o.additionalTabsStr)
		{
//			var addtnlTabJsonArr = JSON.parse("["+o.additionalTabsStr+"]");
			var fixTabsLength = tabs.length;
			for(var i=0;i<o.additionalTabsStr.length;i++)
			{
				var currObj   = o.additionalTabsStr[i];
				
				var addNewTab = {};
				/*tp 985986 Customization Reflection on Custom Added Tabs to Tabbed Pannel*/
				addNewTab.id= currObj.id;
				addNewTab.href     = "#newTabDivId_"+currObj.SUB_OBJECT_ID+"_"+_pageRef;
				addNewTab.label    = currObj.SUB_OBJECT_DESC;
				addNewTab.labelKey = currObj.SUB_OBJECT_DESC;
				addNewTab.disabled = currObj.disabled;
				addNewTab.isVisible = currObj.isVisible;
				addNewTab.cssstyle = currObj.cssStyle;
				tabs.push(addNewTab);
				$("#"+o.id).append("<div id='newTabDivId_"+currObj.SUB_OBJECT_ID+"_"+_pageRef+"'></div>");
			}
		}
		var closable = false;
		var _disabledTabsArr = [];
		var _hiddenTabsArr = [];
		if (tabs) {
			var tabStr = "", l;
			var _firstNotDisAndVis = null;
			/**
			 * [PathSolutions-MarwanMaddah]:Saved Order Management
			 * in case the tabs are sortable and _tabsOrder is defined 
			 * will check the compatibility between the saved arr and the current tabs
			 * and the saved index that are not exists anymore will be removed from arr.
			 * to avoid any conflict on tabs construction
			 * @memberOf {TypeName} 
			 */
			var _tabsOrder = o.tabsOrder;
			var _tabOrderArr = [];
			if(o.sortable && typeof _tabsOrder !="undefined" && _tabsOrder!=null)
			{
               var _tabOrderParseArr = JSON.parse("["+_tabsOrder+"]");
			   var orderArrLength = _tabOrderParseArr.length; 
               if(orderArrLength != tabs.length)
               { 
            	   var _maxInd = 0;
	               for(var j = 0;j<orderArrLength;j++)
				   {
	            	   if(_tabOrderParseArr[j] > _maxInd)
	            	   {
	            		   _maxInd = _tabOrderParseArr[j];
	            	   }
					   if(_tabOrderParseArr[j]<tabs.length)
					   {
						  _tabOrderArr.push(_tabOrderParseArr[j]);   
					   }
				   }
	               if(orderArrLength < tabs.length)
	               {
	            	   var diffLeng = tabs.length - orderArrLength;
	            	   for(var k = 1;k<=diffLeng;k++)
	            	   {
	            		   _tabOrderArr.push(_maxInd + k);
	            	   }
	               } 
               }
               else
               {
            	  _tabOrderArr = _tabOrderParseArr;   
               }
               para.tabOrderArr = o.tabOrderArr = _tabOrderArr;
			}
			for (l = 0; l < tabs.length; l++) {
				/**
				 * [PathSolutions-MarwanMaddah]:
				 * Added to management the tabs order in case of sortable tabbedpanel
				 * 
				 */
				var _pointer = l;
			      /**
			       * [PathSolutions-MarwanMaddah].
			       * in case the same index exists in the order arr will get the related value
			       * to get the tab from tabs table based on it 
			       * and in case not exists will get the tab based on the current for loop index
			       * to avoid the conflict between saved order and the tabs length
			       * (issue appear when added new tabs after save preferences)
			       * @memberOf {TypeName} 
			       */

				  if(_tabOrderArr.length > 0 && typeof _tabOrderArr[l] !="undefined" && _tabOrderArr[l]!=null)
				  { 
				     _pointer = _tabOrderArr[l];
				  }
				/**
				 * [PathSolutions-END] 
				 */
				var tab = tabs[_pointer];
				tabStr += "<li ";
				if (tab.id) {
					tabStr += "id='" + tab.id + "' ";
				}
				/**
				 * [PathSolutions-MarwanMaddah]: added new attribute (tabOrder) to use it on the sortable management 
				 */ 
				tabStr += "tabOrder='"+_pointer+"' ";
				/**
				 * [PathSolutions-MarwanMaddah]:labelKey, to be used in customization management
				 */
				if(tab.labelKey){
					tabStr += "labelKey='" + tab.labelKey + "' ";
				}
				if (tab.cssstyle) {
					tabStr += "style='" + tab.cssstyle + "' ";
				}
				/**
				 * [PathSolution-MarwanMaddah]: in case the tab is disabled. 
				 * the class ui-state-disabled will be added to the clases 
				 */
				var _disabledTab = false;
				if(typeof tab.disabled!="undefined" && tab.disabled !=null && tab.disabled)
				{
				   _disabledTab = true;
				   _disabledTabsArr.push(l);
				}
				/**
				 * [PathSolutions-MarwanMaddah]: save the first not disabled tab 
				 * to use it in case the selected tab is disabled 
				 */
				
				/**
				 * [PathSolutions-MarwanMaddah]: correction to cover the case, 
				 * where the selected Tab is hidden based on the customization management
				 */
				var _hiddenTab = false;
				if(typeof tab.isVisible!="undefined" && tab.isVisible !=null && tab.isVisible == '0')
				{
					_hiddenTab = true;
					_hiddenTabsArr.push(l)
				}
				/**
				 * End
				 */
				else if(_disabledTab == false && _hiddenTab == false && _firstNotDisAndVis == null)
				{
					_firstNotDisAndVis = l;
				}
				if (tab.cssclass) {
					var _classesToAdd = tab.cssclass;
					if(_disabledTab)
					{
					   _classesToAdd = _classesToAdd+" "+"ui-state-disabled";	
					}
					tabStr += "class='" + _classesToAdd + "' ";
				}
                else
                {
                    if(_disabledTab)
                    {
                      tabStr += "class='ui-state-disabled'"; 	
                    }
                }
				/**
				 * [PathSolutions-END] 
				 */
				tabStr += " title='" + tab.label + "'><a href='" + tab.href + "' ";
			...
			/**
			 * [PathSolutions-MarwanMaddah]: in case the defined selected tab is disabled 
			 * the selection will be on the first tab that is not disabled
			 */
			if(typeof para.selected=="undefined" || para.selected==null || para.selected=="")
			{
			  para.selected = 0;	
			}
	        if(_disabledTabsArr.length > 0)
	        {
	           if($.inArray(para.selected,_disabledTabsArr)!= -1 && _firstNotDisAndVis!=null)
	           {
	        	  para.selected = _firstNotDisAndVis;
	           }
	        }	        
	        /**
	         * [PathSolutions-END] 
			 ...
			 * where the selected Tab is hidden based on the customization management
			 */
	        if(_hiddenTabsArr.length > 0)
	        {
	        	if($.inArray(para.selected,_hiddenTabsArr)!= -1 && _firstNotDisAndVis!=null)
	        	{
	        		para.selected = _firstNotDisAndVis;
	        	}
	        }
		...
		if (o.sortable) {
			...
			$elem.find(".ui-tabs-nav").sortable({tabbedId:o.id,axis:'x'});
		}
	...
	// this function is not used anymore all its code is available in date close tag FTL 
	initDatepicker : function(timepicker) {
		...
		if (!self.loadAtOnce) {
			...
			/*if (self.local !== "en") {
				self.require("i18n/jquery.ui.datepicker-" + self.local +  self.minSuffix + ".js");
			}*/
		}
		//PathSolutions Denisk loading datepicker Translation
		if (self.local !== "en")
		{
			self.require("js/base/i18n/ui.datepicker-" + self.local + self.minSuffix + ".js");
		}
		if(timepicker) 
		{
			if (!self.loadAtOnce) {
			...
			//PathSolutions Denisk loading timepicker Translation
			if (self.local !== "en")
			{
				self.require("js/base/i18n/ui.timepicker-" + self.local + self.minSuffix + ".js");
			}
		}
		self.requireCss("js/base/calendars/jquery.calendars.picker.css");
		self.requireCss("js/base/calendars/ui.calendars.picker.css");
		self.require(["/js/base/calendars/jquery.calendars.js","/js/base/calendars/jquery.calendars.plus.js","/js/base/calendars/jquery.calendars.picker.js","/js/base/calendars/jquery.calendars.islamic.js"]);
		if(self.local !== "en")
		{
			self.require(["/js/base/calendars/jquery.calendars.picker-"+self.local+".js","/js/base/calendars/jquery.calendars.islamic-"+self.local+".js"]);
		}
	},
	...
	datepicker : function($elem, o) {
		...
		if(o.timepicker) {
			...
			if (o.year!==undefined) {
				var theDate = new Date(o.year, o.month, o.day, o.hour, o.minute, o.second)//new Date();
				if(theDate.getHours() == 23 && theDate.getDate() != o.day) //in case getHours is 23 and the day is different, means daylight adjustment needed
					theDate.setHours(theDate.getHours() + 2);//just to shift the days when its daylight saving date
				ins.datetimepicker('setDate', theDate);
			}
		}
		else {
			ins.datepicker(params);
			if (o.year!==undefined) {
				//Pathsolutions: used setting of date separately with daylight saving time adjustment because new Date eg: date=30/03/xxxx will be changed to 29/03/xxxx
				var theDate = new Date(o.year, o.month, o.day);//new Date();
				//getHours returns 23 for date 30/03/yyyy, 0 for others
				theDate.setHours(theDate.getHours() > 12 ? theDate.getHours() + 2 : 0);//daylight saving adjustment
				ins.val($.datepicker.formatDate(params.dateFormat, theDate/*new Date(o.year, o.month, o.day)*/ ));
			}
		}
	...
	jquerybutton : function($elem, o) {
		...
		if (o.button) {
			...
			//PathSolutions - set workflow data
			if(o.customBtnData)
			{
				params.customBtnData = o.customBtnData;
			}
			if(o.customBtnAfterExecData)
			{
				params.customBtnAfterExecData = o.customBtnAfterExecData;
			}
			//PathSolutions - set button icon url parameter
			if(o.buttonIconUrl)
			{
				params.buttonIconUrl = o.buttonIconUrl;
			}
