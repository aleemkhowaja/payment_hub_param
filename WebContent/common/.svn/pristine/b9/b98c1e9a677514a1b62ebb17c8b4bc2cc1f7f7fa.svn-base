	...
	function uploadFile(proceed){
		_showProgressBar(true);
		$('#dynaFileForm_' + _pageRef).find('button').attr("disabled", true);
		var fileProcessing = $('#fileProcessing_' + _pageRef).val();
		if(proceed==0 && fileProcessing == '1')
		{
			return;
		}
		$('#fileProcessing_' + _pageRef).val('1');
		...
		{
			$('#fileProcessing_' + _pageRef).val('0');
		...
		$("#importGridSection_" + _pageRef).show();
			...
			var options = { 
				...
				success: function(response, status, xhr) {					
					_showProgressBar(false);
					$('#fileProcessing_' + _pageRef).val('0');
					$('#dynaFileForm_' + _pageRef).find('button').attr("disabled", false);
					try {
