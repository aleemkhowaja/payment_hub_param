...
//			console.log(">>>>>>>>>>>>>>>> DateFormat date "+date)
	...
	$.fn.fmatter.checkbox =function(cval, opts) {
		...
		{
			//on check box click set grid row status for changed or added.
		   var funcName  = opts.colModel.editoptions.dataEvents[0].fn;
		   var rowStat	 = opts.rowId.indexOf('new_')==0?1:2;
		   onClickStr = "onclick=\"$('#"+opts.gid+"').jqGrid('setSelection','"+opts.rowId+"'); $('#"+opts.gid+"').jqGrid('setGridRowStatus','"+opts.rowId+"','"+rowStat+"');  eval("+funcName+").call();\"";
		}
		...
		if (oSelect) {
			...
			if($.fmatter.isObject(oSelect))
			{
				if( $.isFunction(oSelect))
				{
					...
					{ 
						if(typeof opts.colModel.combos[opts.colModel.name] === 'string')
						{
						...
						{
							res = opts.colModel.combos[opts.colModel.name];
						}
					} 
					else 
				    {
					...
					if(res != null && res != "" && typeof res =='string')
					{
						so = res.split(";");				
					}else if(res != null && res != "" && typeof res =='object')
					{
					    so = $.map(res, function(value, index) {
						    return [index+":"+value];
						});
					} 
				}
		...
		if(typeof unformatFunc !== 'undefined' && $.isFunction(unformatFunc) ) {
			...
			switch(formatType) {
				...
				case 'currency':
					...
					break;
				case 'checkbox':
					var cbv = (options.colModel.editoptions && typeof options.colModel.editoptions.value != "undefined") ? options.colModel.editoptions.value.split(":") : ["Yes","No"];
	...
	$.unformat.select = function (cellval,options,pos,cnt) {
		...
		if(op.value){
			...
			if($.isFunction(op.value) )
			{
				...
				{
					if(typeof options.colModel.combos[options.colModel.name] === 'string')
					{
					...
					{
						oSelect = options.colModel.combos[options.colModel.name];
					}
				}
				else
				{
