...
callDependency = (function(dependency, dependencySrc,parameter,elementId, afterDepEvent, dynExpressionsArgs)
{
	...
	assignPrviousValue = function(htmlElemObj)
    {
    	var prevValue = htmlElemObj.attr("prevValue");
    	prevValue = (typeof prevValue == "undefined") ? "" : prevValue ;
		// set the old value and do not proceed to other dependencies
		// if date picker
		if(htmlElemObj.hasClass("hasDatepicker"))
		{
			if(prevValue !== "")
			{
				htmlElemObj.datepicker("setDate", prevValue);
			}
			else
			{
				htmlElemObj.datepicker("setDate", "");
			}
		}
		else
		{
			htmlElemObj.val(prevValue);
		}
    }
	
	$.postJSON(dependencySrc, params, function( data, status, xhr ) 
		{
		    dependencyProcessing = function(confirmedDepProcess)
		    {
		    	if(confirmedDepProcess) // ABAR190059 TP 906700 to make confirmation on dependency 
		    	{
		    	...
		    	else
		    	{
		    		...
		    		//
		    		var $htmlElt = $("#"+inputElemId);
		    		assignPrviousValue($htmlElt);
		    	}
		    }  
		    // ABAR190059 TP 906700 to make confirmation on dependency or not
		    if(data["_dependencyConfirmMsg"] != null )	
		    {
		    	_showConfirmMsg(data["_dependencyConfirmMsg"],confirm_msg_title,"dependencyProcessing","flag");
		    }
		    else
		    {
...
function manageDynamicDisplay(addEltId,addElts,sysParamDisp)
{
	...
	if($element.html() != null || isRadioList)
	{
		...
		var tooltip = addEltEntry["tooltip"];
		var hasTooltip = (null != tooltip && undefined != tooltip && "undefined" != tooltip);
		...
		if(is_visible != null)
		{
			...
			else
			{
				if(is_visible == "0")
				{
					...
					if($element.hasClass("ui-jqgrid-btable"))
					{
						/**
						 * [Sajjad Soomro]
						 * this element will be available in case of dynamic screen
						 * where the position is absolute, so an additional div will be constructed above the grid 
						 * to cover the position absolute
						 */
						_showHideGridCommonCall(addEltId, false);
					}
				}
				else
				{
						...
						else if($element.hasClass("ui-jqgrid-btable"))
						{
							/**
							 * [Sajjad Soomro]
							 * this element will be available in case of dynamic screen
							 ...
							 * to cover the position absolute
							 */
							_showHideGridCommonCall(addEltId, true);
						}
			...
			for(var labelElt in labelsLst)
			{
				...
				if(_$elt.html() != null)
				{
					...
					else //html label
					{
						...
						// TP #945072 - Adding tooltip option via SYS_PARAM_SCREEN_DISPLAY_VO
						_$elt.attr("title", (hasTooltip ? tooltip : _$labelValue));
					}
				}
			}
		}
		...
		else if($element.is("label"))
		{
			...
			// TP #945072 - Adding tooltip option via SYS_PARAM_SCREEN_DISPLAY_VO
			$element.attr("title", (hasTooltip ? tooltip : label_key));
		}
		
		// TP #945072 - Adding tooltip option via SYS_PARAM_SCREEN_DISPLAY_VO
		if(!$element.is("label") && hasTooltip)
		{
...
function _afterValidateErrorMsg(theArgs)
{
	...
	{
		/*tp#965931_Focus on Grid Column*/
		//check if _elementFocusId is for grid col
		var regex = /\w+(#)\w+/g;
		if(regex.test(theArgs["_elementFocusId"]))
		{
			var tempArr = theArgs["_elementFocusId"].split('#');
			var gridId = tempArr[0];
			theArgs["_elementFocusId"] = tempArr[1]; 
			theArgs["_elementFocusId"] = getFirstInvalidGridCellObj( gridId , theArgs["_elementFocusId"]);
		}
		
		var elt = $(document.getElementById(theArgs._elementFocusId));
...
//			if element exists click on it to be opened
		...
		var tooltipserTimeout = setTimeout(function()
		{
			...
			/*tp#965931_Focus on Grid Column*/
			var isEltFocus = elt.is(":focusable,:visible.path-tooltipable");
			if(isEltFocus)
			{
				...
				elt.tooltipster({
					...
					functionAfter: function() { 
						eltId = elt.attr('id');
						if(endsWith(eltId,"_pathTooltipable"))
						{
							elt.attr('id' , eltId.replace("_pathTooltipable",""));
						}
					}
				});
				elt.removeClass("path-tooltipable");
	...
	/**
	 * checks if a grid has a columnName equals to elementName,
	 * and, if found,searches the grid for empty cells in column cells that is related to elementName(ColName);
	 * @returns {string} - id of an element to tooltip in that specific empty-value cell.
	 * 			{string} - elementName argument unchanged if elementName is not a columnName in the grid.
	 */
	function getFirstInvalidGridCellObj(gridId , elementName)
	{
		if(isNotEmptyString(gridId))
		{
			var $grid = $("#"+gridId);
			if($grid.length == 0)
			{
				//try adding progRef to find the grid
				$grid = $("#"+gridId+_pageRef);
			}
			if($grid.length == 0)
			{
				//try adding _progRef to find the grid
				$grid = $("#"+gridId+"_"+_pageRef);
			}
			if($grid.length > 0)
			{
				var colModelObjsArr = $grid.jqGrid('getGridParam').colModel;
				var isGridColName = colModelObjsArr.some( function(col){return col.name === elementName;});
				
				if(isGridColName)
				{
					var data = $grid.jqGrid("getRowData");
					//check for invalid cell for given colName(elementName)
					var invalidRecIndex = -1;
					data.some(function(rec){
						
						var invalidCol = isEmptyString(rec[elementName]);
						if(invalidCol)
						{
							invalidRecIndex = data.indexOf(rec);
						}
					    return invalidCol;
					});
					if(invalidRecIndex >= 0 ) // there is invalid data
					{
						var htmlTableRow = $grid[0].rows.item(invalidRecIndex + 1);//<tr>
						var $htmlTableRow = $(htmlTableRow);
						var $htmlTableCell = $htmlTableRow.children()
												.filter(function(i,e){return e.id.indexOf(elementName) >= 0;/*endsWith(e.id , elementName)*/;})
												.first();//find <td> index that corresponds to elementName(ColName)
						
						elementName = acquireGridCellTooltipElem($htmlTableCell);
...
/**tp#965931_Focus on Grid Column
 * tries to find a focusable element inside a td to tooltip, and in all cases, it returns a ready-to-tooltip id;
 * @param $htmlTableCell - should be td element or jquery object of td element
 * @returns {string} - focusable element id if found
 * 			{string} - td(cell) id if there is no focusable element in td(cell)
 */
function acquireGridCellTooltipElem(htmlTableCell)
{
	var $htmlTableCell = htmlTableCell;
	if(!($htmlTableCell instanceof jQuery))
	{
		if(!($htmlTableCell instanceof Element))
		{
			return "";
		}
	...
	var $firstFocusableElem = $htmlTableCell.find(":input:visible").first();
	if($firstFocusableElem.length == 0) // grid cell doens't have focusable element
	{
		$htmlTableCell.addClass("path-tooltipable");
		var rows = $htmlTableCell.parent().siblings();
		var cellsWithSameId = rows.children().filter(function(i,elem){return elem.id === $htmlTableCell.attr('id');});
		if(cellsWithSameId.length > 0)//if table has other elements with ids equal to $htmlTableCell id
		{
			if(!(endsWith($htmlTableCell.attr('id') , '_pathTooltipable')))//if it doesn't end with _tooltipable
			{
				var thId = $htmlTableCell.attr('id');
				$htmlTableCell.attr('id',thId+"_pathTooltipable");
			}
		}
		return $htmlTableCell.attr('id');
	}
	else // grid cell has focusable element
	{
		$firstFocusableElem.addClass("path-tooltipable");
		return $firstFocusableElem.attr('id');
	}
}
...
function dynamicScreenOnBtnSubmit_confirm(confirm, args)
{
	if (confirm)
	{
		...
		var options = {
				...
				type : 'post',
				dataType: 'json',
				data:{'criteria.screenId':  args.screenId, '_pageRef':_pageRef},
				success : function(data)
				{
					if (data["_error"] == null
...
function dynamicScreen_deleteDynGridRecord(gridId,formId)
{
...
// TP#897711 SUPT190250 - Nazim Jassar ; 29/11/2019 [START]
function dynamicScreen_updateFooterRow(gridId,formId) {
	var colNames = "";
	var theForm = "";
	var dynTableGrid = $("#"+gridId); 
	var footerRow = dynTableGrid.jqGrid('getGridParam','footerrow');
	
	if (footerRow == false || footerRow == undefined)
	{
		return ;
	}
	
	$(dynTableGrid.jqGrid('getGridParam','colModel')).each(function(i)
	{
		if (this.name != 'PRIMARY_COL' && this.name != 'DATE_UPDATED')
		{
			colNames = colNames + this.name + "," ;
		}
	})
	
	if (formId != null && formId != undefined && formId != "") 
	{
		var screenFieldID = parseScreenFieldId(gridId);
		var screenId = parseScreenId(formId);
	
		var gridData =  $("#" + gridId).jqGrid('getAllRows');
		theForm = theForm + "&colNames="+colNames + "&screenFieldID="+screenFieldID+"&gridDetailsData="+gridData + "&screenId="+screenId;
	
		$.ajax({
			 url: jQuery.contextPath+"/path/dynamicScreen/dynamicScreenMainAction_updateDynamicRowFooter",
			 type:"post",
			 dataType:"json",	
			 data: theForm,
			 success: function(data)
			 		{
				 		if(typeof data["_error"] == "undefined" || data["_error"] == null)
				 		{						 			
				 			$("#"+gridId).jqGrid('footerData', 'set', data.rowData , true);
				 		}				 	
				 	}
			 });
	}
...
function parseScreenFieldId(gridId){
	var gridIdWithoutProgRef =  "" 

	if (_pageRef == "" || _pageRef == null)
	{
		gridIdWithoutProgRef = 	gridId;
	}
	else
	{
		gridIdWithoutProgRef = 	gridId.substring(0,gridId.lastIndexOf("_"+_pageRef)); 
	}
	
	return gridIdWithoutProgRef;
}

function parseScreenId(formId){
	var screenId =  "" 

	//screenId = 	formId.substring(10, formId.lastIndexOf("_FormId"));
	if (formId != null && formId != undefined && formId != "")
	{
		screenId = $("#"+formId+"_dynamic_screenId").val();
	}	
...
function dynamicScreen_addDynGridRecord(gridId,formId)
{
	var dynTableGrid = $("#"+gridId); 
	var rowData = {};
	var defaultValueAvailable = dynTableGrid.jqGrid('getGridParam','hasDefaultValueOnAddRow');
	
	if (defaultValueAvailable == false || defaultValueAvailable == undefined )
	{
	...
	_showProgressBar(true);
	var _colModel = dynTableGrid.jqGrid('getGridParam','colModel');
	var colNames = "";
	
	var gridData = $(dynTableGrid).jqGrid('getAllRows');
	...
	if (formId != null && formId != undefined && formId != "") 
	{
		var gridIdWithOutProgRef = parseScreenFieldId(gridId);
		var screenId = parseScreenId(formId);

		var theForm = $("#" + formId).serializeForm();
		theForm = theForm + "&colNames="+colNames + "&screenFieldID="+gridIdWithOutProgRef + "&gridDetailsData=" + gridData + "&screenId="+screenId;
		
		 $.ajax({
			 url: jQuery.contextPath+"/path/dynamicScreen/dynamicScreenMainAction_returnGridColumnDefaultValue",
	         type:"post",
			 dataType:"json",
			 async: false,
			 data: theForm,
			 success: function(data)
			 		{
				 		if(typeof data["_error"] == "undefined" || data["_error"] == null)
				 		{		
				 			var rowId   = dynTableGrid.jqGrid("addInlineRow",data.rowData);
 ...
 * @description used to show/hide jqgrid
 * 
 * @param gridId
 * @param flag
 * @returns
 */
function _showHideGridCommonCall(gridId, flag)
{
	var topParent = $("#"+gridId).closest("div.ui-jqgrid").parent();
	if(flag)
	{
		topParent.css('display','block');
		resizeSingleGrid(gridId);
	}
	else
	{
 ...
 * 
 * @param {any} value
 * @returns {boolean} true - if value is Null-object or undefined.
 ...
 * 
 * @param {any} value
 * @returns {boolean} true - if value isn't undefined and isn't Null-object.
 * 			{boolean} false - otherwise.
 */
 ...
 * 
 * @param {any} value
 * @param {boolean} trimBeforeEvaluate - determines whether to trim value before evaluating.
 * @returns {boolean} true - if value is empty string.
 * 			{boolean} false - if value is string object and not empty.
 * 			{undefined} - if value is not a string object.
 */
function isEmptyString(value, trimBeforeEvaluate)
{
	if(typeof value !== "string")
	{
		return undefined;
	}
	if(trimBeforeEvaluate === true)
	{
		value = value.trim();
	}
	return value === "";
 ...
 * @param {any} value
 * @param {boolean} trimBeforeEvaluate - determines whether to trim value before evaluating.
 * @returns {boolean} false - if value is empty string.
 * 			{boolean} true - if value is string object and not empty.
 * 			{undefined} - if value is not a string object.
 */
function isNotEmptyString(value , trimBeforeEvaluate)
{
	if(typeof value !== "string")
	{
		return undefined;
	}
	return !isEmptyString(value , trimBeforeEvaluate);
}

/**
 * added since IE11 dones't support string.endsWith() function
 * @returns {boolean} true - if both value and endingValue are string objects and value ends with endingValue.
 * {boolean} false - if both value and endingValue are string objects and value doesn't end with endingValue.
 * {undefined} - if either value or endingValue is not string object.
 */
function endsWith(value,endingValue)
{
	if ( $.type(value) === "string" && $.type(endingValue) === "string")
	{
		var pattern = "^[\\s\\S]*"+escapeRegExp(endingValue)+"$";
...
/**
 * added since IE11 dones't support string.startsWith() function
 * @returns {boolean} true - if both value and startingValue are string objects and value starts with startingValue.
 * {boolean} false - if both value and startingValue are string objects and value doesn't start with startingValue.
 * {undefined} - if either value or startingValue is not string object.
 */
function startsWith(value,startingValue)
{
	if ( $.type(value) === "string" && $.type(startingValue) === "string")
	{
		var pattern = "^"+escapeRegExp(startingValue)+"[\\s\\S]*$";
		var regex = new RegExp(pattern,"g");
		return regex.test(value); 
	}
	return undefined;
...
/**
 * escapes Regex special characters in given string
 * @param regexString
 * @returns {string} - regex_escaped string
 * 			regexString - as it is if regexString is not a string object
 */
function escapeRegExp(regexString) 
{
	if($.type(regexString) === "string")
	{
 ...
 * 
 * @param dataToscan
 * @returns {boolean} true - if dataObjToScan is object and has _error as property
 * 			{boolean} false - if dataObjToScan is object and _error is not defined as property
 * 			{undefined} - if dataObjToScan is not an object 
 */
function containsError(dataObjToScan)
{
	if($.type(dataObjToScan) === "object")
	{
		if(isNotNullAndNotUndefined(dataObjToScan["_error"]))
		{
			return true;
		}
		return false;
