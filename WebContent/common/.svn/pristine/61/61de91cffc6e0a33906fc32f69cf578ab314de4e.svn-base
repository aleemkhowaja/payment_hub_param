				...
				{					
				 	...
				 	if(elemntTagName !== "label" && !buttonElement && !collapCont)
					{
				 		...
				 		else if(settingsRight)
						{
							...
							else if(theElement.hasClass("selectCompSize")) //TP#1002338 added Select Box
							{
								elementType = 'fromSelectBox = "1"';
							}
...
function elemCustOpen(theCustIconDiv,custActionType)
{
	...
	//TP#1002338 added fromSelectBox
	var fromSeletBox = iconElem.attr("fromSelectBox")
	...
	//TP#1002338 added Select Box into curParams if not null
	else if(typeof fromSeletBox != "undefined" && fromSeletBox != null)
	{
		curParams.fromSelectBox = fromSeletBox;
	}
...
//	}	
	 ...
	 * if the grid is set to readonly either from development / customization
	 */
	if(!($("#"+relId.replace("gbox_", ""))[0].p.editinline == true || $("#"+relId.replace("gbox_", ""))[0].p.editinline=="true"))
	{
		curParams['readOnlyFlag'] = true;
	}
	var allowDefValCust = iconElem.attr("allowDefValCust");
	if(typeof allowDefValCust != "undefined" && allowDefValCust == "true")
	{
	 ...
	 * 
	 * [TP#1043972] OnChange Event For Grid Column- Editable Grid Customization Enhancements
	 * set fromLiveSearch to 1,0 to handle show/hide of "Custom Activity on Columns" Grid
	 */
    var _addToTitle = "";
		 ...
		 /*953614 Specify Grid Filter Query Expression - Grid/Livesearch Customization Enhancements*/
			 ...
			 validateFilterExpression(relId,function(){
				...
				//in case of activity mapping
					...
					{
						var theGridId = "ObjectCustDetailsGrid_Id_" + currPageRef;
						var $table = $("#"+theGridId);
						var checkRequiredCells = $table.jqGrid('checkRequiredCells');
						if(checkRequiredCells)
						{	
							// check if Column Name is not missing, TP 975144
							var rows = $table.jqGrid('getDataIDs');
							var rowsLen = rows.length;
							for (var j = 0; j < rowsLen; j++)
							{
								var myObject = $table.jqGrid('getRowData', rows[j]);
								var rowObjDetName = myObject["sysParamObjDetailsDispVO.OBJ_DET_NAME"];
								
								if (rowObjDetName == "-1" || rowObjDetName == "" || rowObjDetName == "undefined" || rowObjDetName == null)
								{
									//_showErrorMsg(missing_elt_msg_key, error_msg_title);
									var addArgs = {}; 
									var objNameTooltipIdentifier =  theGridId + "#columnNameComboDesc";
									addArgs["_elementFocusId"] = objNameTooltipIdentifier;
									_showErrorMsg(missing_elt_msg_key,error_msg_title,null ,null, "_afterValidateErrorMsg",addArgs);
									_showProgressBar(false);
									return "invalid";
								}
							}
					...
					/**
					 * [TP#1043972] OnChange Event For Grid Column- Editable Grid Customization Enhancements
					 * In case of custom activity on column get grid json and append to submit
					 */
					if($('#ObjCustElementActivityGrid_Id_' + currPageRef).length > 0)
					{
						var $table = $("#ObjCustElementActivityGrid_Id_" + currPageRef);
						var selectedRowId = $table.jqGrid('getGridParam', 'selrow');
						selectedRowObject = $table.jqGrid('getRowData', selectedRowId);
						var checkRequiredCells = $table.jqGrid('checkRequiredCells');
						if(checkRequiredCells)
						...
						if($('#ObjCustParamMapGrid_Id_' + currPageRef).length > 0 && selectedRowObject!=null && selectedRowObject["sysParamElemActivitiesVO.ACTIVITY_TYPE"] == '6' )
						{
							var $table = $("#ObjCustParamMapGrid_Id_" + currPageRef);
							var checkRequiredCells = $table.jqGrid('checkRequiredCells');
							if(checkRequiredCells)
							{	
								var jsonOpUpdates = $table.jqGrid('getAllRows',false);
...
function fldcust_CheckVisbleRequired(element, what)
{
	...
	var fldcust_fromTab  = $("#fldcust_fromTab_"+custScrPageRef).val();
	// if Required Checked
...
function returnAutoCompleteData(gridId)
{
	var $table = $("#ElementActivityGrid_Id_" + _pageRef);
	if(isNotNullAndNotUndefined(gridId) &&  isNotEmptyString(gridId))
	{
		$table = $("#" + gridId);
	}
	var selectedRowId = $table.jqGrid('getGridParam', 'selrow');
 ...
 * @description open tooltip options for select box
 * 
 * @returns
 */
function openCustValidExpToolTip()
{
	//get element id
	var elemId = returnHtmlEltValue("customId");
	if(typeof _pageRef != "undefined" || _pageRef!=null)
	{
		elemId += "_"+_pageRef;
	}

	$div = $("#fldcust_valid_exp_tooltip_div");
	var tableRelId = $div.children("table").attr("relid");
	if(tableRelId != elemId)
	{
		// iterate select box options and  create table for tooltip
		var contentToAppend = "<table relid='"+elemId+"' width='100%' cellpadding='0' cellspacing='1' border='0'>" +
				"<tr><td class='labelTd'><span>" + escapeHtml(value_valid_exp_pos_values_key) + "</span></td></tr>";
		$("#"+elemId+" option").each(function() {
			contentToAppend += "<tr><td class='labelTd'><span>" + escapeHtml(this.value) + " = " + escapeHtml(this.text) + "</span></td></tr>";
		});
		contentToAppend += "</table>";

		$div.empty();
		$div.append(contentToAppend);
		
		var theDiv = $div.parent();
		var height = theDiv.position().top+20;
		var left = theDiv.position().left;
